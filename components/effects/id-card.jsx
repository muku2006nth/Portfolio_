...
const GLTF_PATH = '/assets/kartu.glb?v=' + Date.now();
const TEXTURE_PATH = '/assets/bandd.png';
const PHOTO_PATH = '/assets/PP.png?v=' + Date.now();
...
function CameraRig() {
  ...
  useEffect(() => {
    camera.position.set(0.5, -0.1, 12.1);
    camera.fov = 21;
...
function Band({ isMobile, maxSpeed = 50, minSpeed = 10 }) {
  ...
  photoTexture.repeat.set(zoom * stretchX, zoom * stretchY);
  photoTexture.offset.set(offsetX, offsetY);
  photoTexture.rotation = rotation;
  ...
  useFrame((state, delta) => {
    if (dragged && card.current && canDrag) {
      ...
      // Add a gentle auto-oscillating breeze
      card.current.wakeUp();
      const time = state.clock.elapsedTime;
      const forceX = Math.sin(time * 1.5) * 0.15;
      const forceZ = Math.cos(time * 1.1) * 0.1;
      card.current.applyImpulse({ x: forceX * delta, y: 0, z: forceZ * delta }, true);
    }
  ...
  return (
    <>
      <group position={[3, 4, 0]}>
        ...
        <RigidBody position={[1.5, 15, 0]} ref={j3} {...segmentProps}><BallCollider args={[0.1]} /></RigidBody>
